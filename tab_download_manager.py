...
class DownloadManagerTab:
    """Enhanced Download Manager with real-time scanning"""
    ...
    def setup_ui(self):
        """Setup complete UI for download manager"""
        # Title
        ...
        ttk.Label(stats_frame, textvariable=self.total_downloads_var,
                  font=("Arial", 10)).pack(side="left", padx=10)
        ttk.Label(stats_frame, textvariable=self.active_downloads_var,
                  foreground="#2196F3", font=("Arial", 10, "bold")).pack(side="left", padx=10)
        ttk.Label(stats_frame, textvariable=self.blocked_downloads_var,
                  foreground="#d32f2f", font=("Arial", 10)).pack(side="left", padx=10)
        ttk.Label(stats_frame, textvariable=self.safe_downloads_var,
                  foreground="#2e7d32", font=("Arial", 10)).pack(side="left", padx=10)
        
        # Create notebook for different views
        self.download_notebook = ttk.Notebook(self.frame)
        self.download_notebook.pack(fill="both", expand=True, padx=10, pady=5)
        
        # Tab 1: Detected Downloads
        ...
        ttk.Button(control_frame, text="🗑️ Clear Detected",
                   command=self.clear_detected_list).pack(side="left", padx=5)
        ttk.Button(control_frame, text="📂 Open Downloads Folder",
                   command=self.open_downloads_folder).pack(side="left", padx=5)
    ...
    def setup_detected_tab(self, parent):
        ...
        self.detected_table = ttk.Treeview(table_frame, columns=columns,
        ...
        self.detected_table.tag_configure("blocked", background="#ffe6e6",
                                         foreground="#d32f2f")
        self.detected_table.tag_configure("scanning", background="#fff9e6")
        
        scrollbar = ttk.Scrollbar(table_frame, orient="vertical",
                                 command=self.detected_table.yview)
    ...
    def setup_manual_tab(self, parent):
        ...
        self.manual_table = ttk.Treeview(table_frame, columns=columns,
                                        show="headings", height=18)
        ...
        self.manual_table.tag_configure("downloading", background="#e3f2fd",
                                       foreground="#1976D2")
        self.manual_table.tag_configure("completed", background="#e8f5e8",
                                       foreground="#2e7d32")
        self.manual_table.tag_configure("failed", background="#ffe6e6",
                                       foreground="#d32f2f")
        self.manual_table.tag_configure("paused", background="#fff9e6")
        self.manual_table.tag_configure("blocked", background="#ffcdd2",
                                       foreground="#c62828")
        
        scrollbar = ttk.Scrollbar(table_frame, orient="vertical",
                                 command=self.manual_table.yview)
    ...
    def show_add_download_dialog(self):
        ...
        ttk.Label(main_frame, text="Download URL:",
        ...
        def paste_url():
            try:
                url_entry.insert(0, dialog.clipboard_get())
            except:
                pass
        
        ttk.Button(url_frame, text="📋 Paste",
                   command=paste_url).pack(side="right")
        
        ttk.Label(main_frame, text="Save Location:",
        ...
        ttk.Checkbutton(security_frame,
                       text="🛡️ Scan with VirusTotal before download",
                       variable=scan_var).pack(anchor="w", pady=2)
        ttk.Checkbutton(security_frame,
                       text="🚫 Auto-block if threat detected (>30% malicious)",
    ...
    def download_file_thread(self, download_info):
        ...
        try:
            ...
            if download_info["scan_first"]:
                ...
                self.frame.after(0, self.refresh_manual_downloads)
                ...
                try:
                    ...
                    if "error" in scan_result:
                        ...
                        suspicious_count = scan_result.get("suspicious", 0)
                        total = scan_result.get("total", 1)
                        threat_count = malicious_count + suspicious_count
                        percentage = (threat_count / total * 100) if total > 0 else 0
                        
                        if percentage > 30:
                            download_info["status"] = "BLOCKED"
                            download_info["safety"] = f"⚠️ {threat_count}/{total} flagged"
                            self.frame.after(0, self.refresh_manual_downloads)
                            self.frame.after(0, lambda: messagebox.showwarning("Download Blocked",
                                                                               f"Download blocked for security reasons:\n\n"
                                                                               f"• {threat_count} out of {total} scanners flagged this URL\n"
            ...
            self.frame.after(0, lambda: messagebox.showinfo("Download Complete",
            ...
            if "cancelled" not in error_msg.lower():
                self.frame.after(0, lambda: messagebox.showerror("Download Failed",
                                                                 f"Download failed:\n\n{error_msg}\n\n"
    ...
    def open_downloads_folder(self):
        ...
        if os.path.exists(default_path):
            if os.name == 'nt':
                ...
                os.system(f'xdg-open "{default_path}"')
        else:
